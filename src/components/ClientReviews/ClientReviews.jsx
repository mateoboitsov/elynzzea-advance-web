...
const ClientReviews = () => {
  ...
  const animateImageChange = (newImageSrc) => {
    ...
    return gsap.to(newImg, {
      ...
      ease: "power2.out",
      onComplete: () => {
        // Solo eliminar imágenes que están directamente en client-review-img, no las de clients-list
        const clientsList = imageContainerRef.current.querySelector(".clients-list");
        const allImages = imageContainerRef.current.querySelectorAll("img");
        allImages.forEach((img) => {
          // No eliminar si la imagen está dentro de clients-list o si es la nueva imagen
          if (img !== newImg && (!clientsList || !clientsList.contains(img))) {
            img.remove();
          }
        });
      },
